...
def prelim_agent(house: housebuilder.House, room0_temp: float, room0_setp: float, room1_temp: float, room1_setp: float):
	if room0_temp > room0_setp + house.constants.epsilon and room1_temp > room1_setp:
		return (house.constants.settings.index(-1), [[False, False]])
	if room0_temp < room0_setp - house.constants.epsilon and room1_temp < room1_setp:
		return (house.constants.settings.index(1), [[False, False]])
	
	if room0_temp > room0_setp and room1_temp > room1_setp + house.constants.epsilon:
		return (house.constants.settings.index(-1), [[False, False]])
	if room0_temp < room0_setp and room1_temp < room1_setp - house.constants.epsilon:
		return (house.constants.settings.index(1), [[False, False]])

	if room0_temp > room0_setp + house.constants.epsilon and abs(room1_temp - room1_setp) < house.constants.epsilon:
		return (house.constants.settings.index(-1), [[False, True]])
	if room1_temp > room1_setp + house.constants.epsilon and abs(room0_temp - room0_setp) < house.constants.epsilon:
		return (house.constants.settings.index(-1), [[True, False]])
	...
	if room0_temp > room0_setp + house.constants.epsilon and room1_temp < room1_setp - house.constants.epsilon:
